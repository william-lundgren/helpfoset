...
class dictionary:
    ...
    def add_user(self, name: str, sort: str, group: list[str, str]):
        ...
        if name == None or sort == None or group == None:
                raise TypeError('expected name, sort and group')
        
        if not self.contains(self.user_dictionary, name):
            if sort == 'Nolla':
                if group[0] == ' ':
                    raise ValueError('No mentor group provided')
                if group[1] == ' ':
                    raise ValueError('No mission group provided')
            ...
            raise RuntimeError('User already in system') 
        
    def add_from_list(self, groups_file_name):
        with open(groups_file_name, 'r', encoding='UTF-8-SIG') as f:
            for line in f.readlines():
                line = line[:-1]
                name,sort, group, mission = line.split(';')
                try:
                    self.add_user(name.lower(), sort, [group, mission])
                except RuntimeError:
                    print('User ' + name + ' already in system')
...
# Lägg till personer från faddergruppsregistret
dic.add_from_list('grupper.csv')
